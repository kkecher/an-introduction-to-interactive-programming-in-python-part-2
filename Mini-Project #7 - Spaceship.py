...
explosion_info = ImageInfo(center = [64, 64], size = [128, 128], radius = 17, lifespan = 24, animated = True)
...
def process_sprite_group(rock_group, canvas):
    ...
    for rock in rock_group_copy:
        ...
        if is_in_game and rock.collide(my_ship):
            explosion = Sprite(pos = rock.get_pos(), vel = [0, 0], ang = 0, ang_vel = 0, image = explosion_image, info = explosion_info, sound = explosion_sound)
            explosion_group.add(explosion)
            rock_group.discard(rock)
            lives -= 1
            if lives <= 0:
                for i in range(300): # more explosions to god of exlosions!
                    offset = random.randrange(-30, 30)
                    explosion = Sprite(pos = [x + offset for x in my_ship.get_pos()], vel = [0, 0], ang = 0, ang_vel = 0, image = explosion_image, info = explosion_info, sound = explosion_sound)
                    explosion_group.add(explosion)
        if lives <= 0:
